...
export function ProductCard({ p }: { p: CatalogProduct }) {
  ...
  return <article className="card product-card">
    <Link className="product-card-link" href={`/products/${p.slug}`} aria-label={`查看${p.name}详情`}>
      ...
      <div className="product-card-copy"><span className="tag">{p.type}</span><h3>{p.name}</h3><div className="product-card-brand">{p.brand}</div>{p.variants.length > 0 && <div className="product-meta">{[...new Set(p.variants.map((variant) => variant.size))].join(" · ")}</div>}</div>
    </Link>
