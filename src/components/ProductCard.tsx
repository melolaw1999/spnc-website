...
export function ProductCard({ p }: { p: CatalogProduct }) {
  ...
  return <article className="card product-card">
    <Link href={`/products/${p.slug}`} aria-label={`查看${p.name}详情`}>
      ...
      <span className="tag">{p.type}</span><h3>{p.name}</h3><div className="muted">{p.brand}</div><p>{p.summary}</p><div className="product-meta">{[...new Set(p.variants.map((variant) => variant.size))].join(" · ")}</div>
    </Link>
    <Link className="text-link" href={`/products/${p.slug}`}>查看产品与购买 →</Link>
  </article>;
}
