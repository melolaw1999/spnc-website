...
export default function Contact() {
  return <main className="section"><div className="container narrow"><div className="eyebrow">Contact</div><h1 className="page-title">联系我们</h1><p className="lead detail-lead">品牌合作、售后咨询与商务沟通，请按事项选择邮箱。购买仍在淘宝店完成。</p><div className="contact-grid">{enterpriseContacts.map((contact) => <a className="card contact-card" href={mailto(contact.email)} key={contact.email}><h2 className="minor-title">{contact.role}</h2><strong>{contact.email}</strong><p className="muted">{contact.note}</p></a>)}</div><div className="contact-grid"><div className="card"><h2 className="minor-title">购买咨询</h2><p className="muted">在淘宝店查看实时在售规格、口味、价格与库存。</p><TaobaoButton label="打开理想营养淘宝店" /></div><div className="card"><h2 className="minor-title">订单售后</h2><p className="muted">退换、破损处理建议从原淘宝订单发起，便于保留完整记录。</p><TaobaoButton label="前往淘宝店" secondary /></div></div><div className="notice contact-notice">理想营养不会索取淘宝密码、支付密码或短信验证码，也不会要求脱离淘宝订单私下转账。</div></div></main>;
}
